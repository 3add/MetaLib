...
public class EntityManager {
    ...
    public void addEntity(ProtocolEntity entity) {
        ...
        Vector3d pos = entity.getTrackedEntity().getTrackedState().currentPos();
        this.chunkTracker.add(entity, pos.getX(), pos.getZ());
        this.entitiesByWorld
                .computeIfAbsent(entity.getTrackedEntity().getTrackedState().currentWorld(), w -> ConcurrentHashMap.newKeySet())
                .add(entity);
    }
    ...
    public void removeEntity(ProtocolEntity entity, boolean despawn) {
        ...
        Set<ProtocolEntity> worldSet = this.entitiesByWorld.get(entity.getTrackedEntity().getTrackedState().currentWorld());
        if (worldSet != null) {
            worldSet.remove(entity);
            if (worldSet.isEmpty()) this.entitiesByWorld.remove(entity.getTrackedEntity().getTrackedState().currentWorld());
        }
