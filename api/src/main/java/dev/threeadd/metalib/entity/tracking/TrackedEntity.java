 ...
 * @see ProtocolEntity
 * @see TrackedEntityState
 */
@ApiStatus.Internal
public class TrackedEntity {

    private final ProtocolEntity entity;
    private TrackedEntityState trackedPosition;

    public TrackedEntity(ProtocolEntity entity, Location initialLocation,
                         @Nullable Vector3d initialVelocity, float initialVerticalHeadRot,
                         boolean initialOnGround, ProtocolWorld initialWorld) {
        this.entity = entity;
        this.trackedPosition = new TrackedEntityState(
    ...
    public TrackedEntityState getTrackedState() {
        return this.trackedPosition;
    }

    public void teleport(Location location) {
        updatePosition(location.getPosition(), location.getYaw(), location.getPitch(),
                location.getYaw(), this.trackedPosition.currentOnGround(),
                this.trackedPosition.currentWorld());
    }

    public void setYaw(float yaw) {
        updatePosition(this.trackedPosition.currentPos(), yaw, this.trackedPosition.currentPitch(),
                yaw, this.trackedPosition.currentOnGround(), this.trackedPosition.currentWorld());
    }

    public void setPitch(float pitch) {
        updatePosition(this.trackedPosition.currentPos(), this.trackedPosition.currentYaw(), pitch,
                this.trackedPosition.currentVerticalHeadRot(), this.trackedPosition.currentOnGround(),
                this.trackedPosition.currentWorld());
    }

    public void setVerticalHeadRot(float verticalHeadRot) {
        updatePosition(this.trackedPosition.currentPos(), this.trackedPosition.currentYaw(),
                this.trackedPosition.currentPitch(), verticalHeadRot,
                this.trackedPosition.currentOnGround(), this.trackedPosition.currentWorld());
    }

    public void setOnGround(boolean onGround) {
        updatePosition(this.trackedPosition.currentPos(), this.trackedPosition.currentYaw(),
                this.trackedPosition.currentPitch(), this.trackedPosition.currentVerticalHeadRot(),
                onGround, this.trackedPosition.currentWorld());
    }

    public void setVelocity(Vector3d velocity) {
        this.entity.sendPacketsToViewers(new WrapperPlayServerEntityVelocity(this.entity.getEntityId(), velocity));
        this.trackedPosition = this.trackedPosition.withVelocity(velocity);
    }

    public void setWorld(ProtocolWorld newWorld) {
        ProtocolWorld oldWorld = this.trackedPosition.currentWorld();
        if (oldWorld.equals(newWorld)) return;

        Vector3d pos = this.trackedPosition.currentPos();

        MetaLib.getAPI().getEntityManager().updateEntityWorld(
                this.entity, oldWorld, pos, newWorld, pos
        );

        this.trackedPosition = this.trackedPosition.withWorld(newWorld);
    ...
    public void lookAt(Vector3d target) {
        ...
        this.trackedPosition = this.trackedPosition.syncWith(
                this.trackedPosition.currentPos(),
                yaw, pitch, yaw,
                this.trackedPosition.currentOnGround(),
                this.trackedPosition.currentWorld()
        );
    ...
    private void updatePosition(Vector3d position, float yaw, float pitch,
        ...
        updatePosition(this.trackedPosition.syncWith(position, yaw, pitch, verticalHeadRot, onGround, world));
    }

    private void updatePosition(TrackedEntityState position) {
        Vector3d oldPos = this.trackedPosition.currentPos();
        ...
        if (oldChunkX != newChunkX || oldChunkZ != newChunkZ) {
            MetaLib.getAPI().getEntityManager().updateEntityChunk(this.entity, oldPos.getX(), oldPos.getZ(), newPos.getX(), newPos.getZ());
        }
    ...
    private void dispatchMovementUpdates() {
        ...
        if (this.trackedPosition.needsFullSync()) {
            this.entity.sendPacketsToViewers(
                    createTeleportPacket(),
                    new WrapperPlayServerEntityHeadLook(this.entity.getEntityId(),
                            this.trackedPosition.currentVerticalHeadRot())
            );
            markSynced();
            return;
        }

        boolean positionChange = this.trackedPosition.hasPositionChanged(0.0002);
        boolean pitchYawChange = this.trackedPosition.hasPitchYawChanged(0.05f);
        boolean verticalHeadRotChange = this.trackedPosition.hasVerticalHeadRotChanged(0.05f);
        boolean groundChanged = this.trackedPosition.hasOnGroundChanged();
        ...
        if (this.trackedPosition.hasPositionChanged(7.999755859375)) {
            this.entity.sendPacketsToViewers(createTeleportPacket());
        } else if (positionChange && pitchYawChange) {
            Vector3d deltaPos = this.trackedPosition.deltaPosition();
            this.entity.sendPacketsToViewers(new WrapperPlayServerEntityRelativeMoveAndRotation(entityId,
                    ...
                    this.trackedPosition.currentYaw(), this.trackedPosition.currentPitch(),
                    this.trackedPosition.currentOnGround()
            ));
            ...
            this.entity.sendPacketsToViewers(new WrapperPlayServerEntityRelativeMove(entityId,
                    deltaPos.getX(), deltaPos.getY(), deltaPos.getZ(), this.trackedPosition.currentOnGround()
            ));
        } else if (pitchYawChange) {
            this.entity.sendPacketsToViewers(new WrapperPlayServerEntityRotation(entityId,
                    this.trackedPosition.currentYaw(), this.trackedPosition.currentPitch(), this.trackedPosition.currentOnGround()
            ));
        }
    ...
    public void markSynced() {
        this.trackedPosition = this.trackedPosition.sync();
    }
    ...
    private PacketWrapper<?> createTeleportPacket() {
        if (IS_NEWER_THAN_1_21_2) {
            return new WrapperPlayServerEntityPositionSync(this.entity.getEntityId(),
                    this.trackedPosition.asEntityPosData(), this.trackedPosition.currentOnGround()
            );
        }
        ...
        return new WrapperPlayServerEntityTeleport(this.entity.getEntityId(),
                this.trackedPosition.asLocation(), this.trackedPosition.currentOnGround());
    }
    ...
    public PacketWrapper<?> createSpawnPacket() {
        ...
        if (type == EntityTypes.PLAYER) {
            return new WrapperPlayServerSpawnPlayer(
                    ...
                    this.entity.getUuid(),
                    this.trackedPosition.asLocation(),
        ...
        if (type == EntityTypes.EXPERIENCE_ORB) {
            ...
            return new WrapperPlayServerSpawnExperienceOrb(
                    ...
                    pos.getY(),
                    pos.getZ(),
                    this.entity.getMeta().getOptional(ExperienceOrbMetaProperties.VALUE)
...
//                );
            ...
            if (type.isInstanceOf(EntityTypes.LIVINGENTITY)) {
                Vector3d velocity = this.trackedPosition.velocity();
                return new WrapperPlayServerSpawnLivingEntity(
                        this.entity.getEntityId(),
                        this.entity.getUuid(),
                        type,
                        this.trackedPosition.currentPos(),
                        this.trackedPosition.currentYaw(),
                        this.trackedPosition.currentPitch(),
                        this.trackedPosition.currentVerticalHeadRot(),
