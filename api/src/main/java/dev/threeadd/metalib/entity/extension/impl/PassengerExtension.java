...
import dev.threeadd.metalib.entity.event.ViewerShowEntityEvent;
import dev.threeadd.metalib.entity.extension.EntityExtension;
import dev.threeadd.metalib.util.PacketDispatcher;
...
public class PassengerExtension implements EntityExtension {
    ...
    private void handleSpawn(EntitySpawnEvent event) {
        ...
        ProtocolEntity vehicle = MetaLib.getAPI().getEntityManager().getEntity(this.riding);
    ...
    private void handleDespawn(EntityDespawnEvent event) {
        ...
        for (int passengerId : this.passengers) {
            ProtocolEntity passenger = MetaLib.getAPI().getEntityManager().getEntity(passengerId);
            if (passenger != null) {
    ...
    public PassengerExtension startRiding(int vehicleId) {
        ...
        ProtocolEntity vehicle = MetaLib.getAPI().getEntityManager().getEntity(vehicleId);
        if (vehicle != null && vehicle.hasExtension(PassengerExtension.class)) {
